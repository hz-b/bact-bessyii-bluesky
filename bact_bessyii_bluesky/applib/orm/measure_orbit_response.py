...
from bact_bessyii_ophyd.devices.pp import bpm, multiplexer, steerers
...
def main(prefix, currents,machine_name,catalog_name, measurement_name,try_run=False):
    ...
    mux = steerers.SteererCollection(prefix, name="sc")
    ...
    # test hack ...
    steerer_names = steerer_names
    if try_run:
        steerer_names = steerer_names[:2]
    lt = LiveTable(
        [
            mux.sel.selected.name,
            mux.sel.name + "_p_setpoint",
            "sc_sel_r_setpoint",
            "sc_sel_p_readback",
            "sc_sel_r_readback",
            cs.name,
    ...
    # plot_list = []
    # for elem in bpm_read["bpm_elem_data"]['value']:
    plot = orbit_plots.plots(magnet_name=mux.sel.selected.name,
                             ds=None,
    ...
    currents = np.array([0, -1, 0, 1, 0]) * 1e-3
